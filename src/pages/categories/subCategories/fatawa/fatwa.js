...
const Fatwa = ({ src, audioName }) => {
    ...
    const fetchCategories = async () => {
      ...
      try {
        ...
        if (data.sub_categories && data.sub_categories.length > 3) {
          setCategories(
            data.sub_categories[3].sub_categories[13].sub_categories
          ); // Only use subcategory 8
        }
      } catch (error) {
  ...
  const ModalContent = () => {
    return (
      <Box sx={{ p: 1 }} className="fatawa-component">
        <Box
          sx={{
            display: "flex",
