...
const Animals = ({ scrollUp }) => {
  ...
  const handleStoryClick = (e, story, index) => {
    ...
    localStorage.setItem("selectedAnimalStory", JSON.stringify(story));
    localStorage.setItem("selectedAnimalCardIndex", index);
  };
  ...
  useEffect(() => {
    const savedStory = localStorage.getItem("selectedAnimalStory");
    const savedIndex = localStorage.getItem("selectedAnimalCardIndex");
  ...
  return (
    <div className="single_story" ref={content}>
      ...
      <hr />
      <h2 className="title_pages flex mt-4">
        {selectedStory
          ? language === "ar"
            ? "--' المزيد من القصص '--"
            : "--' More Stories '--"
      ...
      <div className="suggest" ref={suggests}>
        ...
        {stories.animals.map((animal, index) => (
          <div
            key={index}
            className={`story-item card rounded-1 ${
              selectedCardIndex === index ? "selected" : ""
            }`}
            onClick={(e) => handleStoryClick(e, animal, index)} // Update selected story on click
          >
            <section
              className="item w-100 pe-none"
              title={language === "ar" ? animal.name.ar : animal.name.en}
            >
              <div className="w-100 position-relative">
                ...
                <img
                  ...
                  src={hasError ? defaultImage : animal.image}
                  alt="storyLogo"
              ...
              <h2 className="title">
                {language === "ar" ? animal.name.ar : animal.name.en}
              </h2>
            </section>
          </div>
        ))}
